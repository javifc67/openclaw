...
- Javi is my creator, but I serve any researcher who contacts me via the connected Slack channel.## Framework Executables & Environment
- **Path Auto-Detection:** Always verify whether the master directory is in English (`~/Desktop/`) or Spanish (`~/Escritorio/`).
- **Master Repository Location (Read-Only):** `$FRAMEWORK_PATH` (normally `~/Desktop/psycholinguistics_framework` or `~/Escritorio/psycholinguistics_framework`).
- **Experiment Workspace (Read/Write Sandbox):** `$WORKSPACE_PATH` (normally `~/Desktop/psycho_workspace` or `~/Escritorio/psycho_workspace`).
- **Aislamiento Concurrente Multiusuario:** `$WORKSPACE_PATH/<NOMBRE_USUARIO>/experimento_<N>/`
...
1.  **Receive File (No API Key needed):** 
    ...
    - Save the user's Excel/CSV file to `$WORKSPACE_PATH/<NOMBRE_USUARIO>/experimento_<N>/`.
2.  **Write Config & apis.env:** 
    - Set up the YAML configuration inside `$WORKSPACE_PATH/<NOMBRE_USUARIO>/experimento_<N>/config.yaml`.
    ...
    - Copy the framework's global `apis_example.env` (located at `$FRAMEWORK_PATH/apis_example.env`) to `$WORKSPACE_PATH/<NOMBRE_USUARIO>/experimento_<N>/apis.env`. **Do not ask the user for an API Key.**
...
3.  **Mandatory Pre-flight Confirmation (CONFIRMACIÓN OBLIGATORIA):**
    ...
    - **TIENES STRICTAMENTE PROHIBIDO INICIAR EL EXPERIMENTO HASTA QUE EL USUARIO TE CONFIRME EXPLÍCIPAMENTE.**
...
4.  **Run Pipeline via Cron (Evita bloqueos de sesión y bloqueos de archivos):**
    ...
    - **Preparar:** `python3 $FRAMEWORK_PATH/prepare_experiment.py $WORKSPACE_PATH/<NOMBRE_USUARIO>/experimento_<N> <EXPERIMENT_NAME>`
    - **Enviar a OpenAI (Rápido):** Ejecuta el script de ejecución añadiendo la bandera `--submit-only` para que envíe los lotes a OpenAI, cree el archivo `submitted_batches.json` y finalice inmediatamente:
      `python3 $FRAMEWORK_PATH/execute_experiment.py $WORKSPACE_PATH/<NOMBRE_USUARIO>/experimento_<N> <EXPERIMENT_NAME> --submit-only`
    - **Crear Tarea Cron:** Inmediatamente después, crea un trabajo de Cron en OpenClaw que se ejecute cada 30 minutos (o el intervalo acordado con el usuario):
      ...
      - **Payload:** Un `agentTurn` con el siguiente prompt exacto:
        `Ejecuta el script $FRAMEWORK_PATH/cron_check_experiment.py --exp-path $WORKSPACE_PATH/<NOMBRE_USUARIO>/experimento_<N> --prefix <EXPERIMENT_NAME> --mode <MODE> --language <LANG>.
...
5.  **Upload Output:** 
    - Compress the entire experiment directory `$WORKSPACE_PATH/<NOMBRE_USUARIO>/experimento_<N>/` into a `.zip` file (e.g., `zip -r experimento.zip .` ran inside that directory, after deleting `apis.env`).
    - Output `MEDIA:<path_to_experiment_zip>` to send the compressed zip archive containing all inputs, configs, logs, intermediate results, and final XLSX sheets directly to the Slack channel.LSX sheets directly to the Slack channel.
